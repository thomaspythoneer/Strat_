...
;; creating a "games" map to map GameID => the Game Tuple
;; player-one is the creator of the game.
;; player-two isn't declared util someone joins the game, als we don't have a winner until someone actually wins - so we just keep those optional.
;; board: is a list of 9 elements of type uint. We will use different numbers to represent whether each cell in the board is empty, an X, or an O.
(define-map games 
    uint ;; Key (Game ID)
    { ;; Value (Game Tuple)
...
;; For each cell in the board, we'll assign special meaning to the numbers 0, 1 and 2.

;; We'll take 0 to mean that the cell is EMPTY. 1 implies the cell has an X in it. And 2 implies the cell has an O in it.
(define-private (validate-move (board (list 9 uint)) (move-index uint) (move uint))
    (let (
        ;; Validate that the move is being played within range of the board i.e between 0-9
